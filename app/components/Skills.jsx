...
const Skill = ({ name, x, y }) => {
    return (
        <motion.div className="flex items-center justify-center rounded-full font-semibold bg-light text-dark py-3 px-6 shadow-dark cursor-pointer absolute md:py-2 md:px-4 ss:text-ss ss:py-1.5 ss:px-3 xs:text-dark  xs:font-bold" whileHover={{ scale: 1.05 }} initial={{ x: 0, y: 0 }} whileInView={{ x: x, y: y, transition: { duration: 1.5 } }} >
            {name}
        </motion.div>
    )
}
...
const Skills = () => {
    return (
        <section className={`mb-20`} id="product">
            <h2 className="font-bold text-8xl mt-64 w-full text-center text-white sm:text-6xl sm:mt-32">Skills</h2>
